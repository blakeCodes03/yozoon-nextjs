...
export async function sellUserTokens({
  ...
  try {
    ...
    try {
      ...
      throw new Error(
        '❌ Seller ATA does not exist — cannot sell without tokens.'
      );
    }
    ...
    // 2. Fetch the account
    const userStateAccount = await (program.account as any)['userState'].fetch(
      sellerUserStatePDA
    );
    ...
    const sellIx = await program.methods
      ...
      .accounts({
        ...
        referrer: referrerAccount ?? PublicKey.default,
        platformTreasury,
