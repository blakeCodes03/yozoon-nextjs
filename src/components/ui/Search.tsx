  ...
  return (
    <div className="  xs:col-span-4 sm:col-span-6 md:col-span-6 lg:col-span-5 xl:col-span-3 order-2 sm:order-2 md:order-2 lg:order-2 xl:order-3 ring-2 ring-[#FFB92D]  rounded-lg focus:outline-none focus-within:ring-0 w-full relative  ">
      <div className="relative   ">
        ...
        />
        <input
          className=" dark:bg-[#181A20] bg-bglight robboto-fonts placeholder:text-sm dark:placeholder:text-[#A6A6A6] placeholder:text-gray-800 dark:text-[#A6A6A6] text-black text-sm  inter-fonts relative w-full h-11 rounded-lg pl-9 pr-34 focus:outline-none "
          type="search"
          placeholder="Token"
          value={query}
      ...
      <div className="absolute w-full mt-2 top-full left-0 z-20">
        ...
        <ScrollArea
          ref={observerRef}
          className={cn(
            'w-full mt-1 dark:bg-[#1E2329] bg-bglight shadow-lg rounded-lg transition-all',
            coins.length < 2
              ? 'h-[4.5rem]'
              : coins.length < 3
                ? 'h-[9.5rem]'
        ...
        >
          ...
          <ul className="divide-y divide-gray-800">
            ...
            {coins.map((coin) => (
              <li
                ...
                className="flex items-center justify-between px-4 py-3 dark:hover:bg-[#2A2F36] hover:bg-blue-gray-400 transition-colors"
              >
                {/* Image */}
                <div className="flex items-center space-x-4">
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className="dark:text-gray-400 text-gray-800 text-xs">{coin.ticker}</span>
                  </div>
                </div>
                {/* Market Cap */}
          ...
          {(loading || !hasMore) && (
            <div className=" text-black dark:text-white flex items-center justify-center content-center w-full h-[4.5rem]">
              {/* Loading Indicator */}
